...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{dyn_fig00_flight_routes.png}
    \caption{Geographical overview of the 27 evaluation flight routes across Europe.}
    \label{fig:flight_routes}
\end{figure}
...
This is confirmed by the global fuel flow MAE: $7{,}284.15 \pm 1{,}167.60$\,kg/h for BlueSky, $1{,}783.46 \pm 932.76$\,kg/h for BADA~3, and $294.37 \pm 349.27$\,kg/h for BADA~4. Even relative to BADA~3, BADA~4 achieves an $83.5\%$ reduction in fuel flow MAE.
...
\begin{enumerate}
    \item \textbf{Altitude}: BlueSky achieves $611.83 \pm 596.81$\,ft, BADA~3 (pyBADA) $664.42 \pm 637.07$\,ft, and BADA~4 (pyBADA) $671.89 \pm 643.29$\,ft. The comparable values across all models confirm the passive-observer nature of MODE~0: trajectory is effectively identical regardless of the performance model.
    \item \textbf{Vertical speed}: BlueSky $311.31 \pm 311.41$\,ft/min, BADA~3 (pyBADA) $302.19 \pm 290.38$\,ft/min, BADA~4 (pyBADA) $304.24 \pm 296.17$\,ft/min. All comparable, confirming shared kinematic autopilot.
    \item \textbf{True airspeed}: BlueSky $12.58 \pm 11.79$\,kt, BADA~3 (pyBADA) $17.38 \pm 16.96$\,kt, BADA~4 (pyBADA) $16.10 \pm 14.85$\,kt. The slightly higher pyBADA errors reflect stricter speed schedule enforcement.
    \item \textbf{Thrust}: The most significant improvement --- BlueSky $15{,}322.84 \pm 2{,}494.29$\,daN, BADA~3 (pyBADA) $3{,}945.22 \pm 1{,}957.47$\,daN, BADA~4 (pyBADA) $662.96 \pm 840.57$\,daN. BADA~4 (pyBADA) reduces thrust MAE by $95.7\%$ vs BlueSky and by $83.2\%$ vs BADA~3 (pyBADA).
    \item \textbf{Fuel flow}: BlueSky $7{,}284.15 \pm 1{,}167.60$\,kg/h, BADA~3 (pyBADA) $1{,}783.46 \pm 932.76$\,kg/h, BADA~4 (pyBADA) $294.37 \pm 349.27$\,kg/h. BADA~4 achieves a $96\%$ reduction vs BlueSky and an $83.5\%$ reduction vs BADA~3 (pyBADA).
    \item \textbf{Mass}: BlueSky $218{,}998.07 \pm 7{,}876.98$\,kg (dominated by zero fuel depletion and initial mass mismatch), BADA~3 (pyBADA) $1{,}936.57 \pm 823.02$\,kg, BADA~4 (pyBADA) $154.09 \pm 168.39$\,kg. BADA~4 (pyBADA) reduces mass MAE by $>99\%$ vs BlueSky.
\end{enumerate}

Overall, the \texttt{DYNAMIC\_BADA} integration in MODE~0 successfully resolves the fundamental thermodynamic limitations of the baseline BlueSky simulator for all engine performance variables. By implementing the BADA aerodynamic and engine performance models within BlueSky's performance interface, the plugin enforces physically realistic speed envelopes, computes physically consistent thrust and fuel flow for each flight phase, models fuel depletion with mass feedback, and exposes performance coefficients for external analysis. As a passive observer in MODE~0, the plugin does not override BlueSky's geometric trajectory computation; the trajectory continues to be driven by BlueSky's kinematic autopilot, which explains why altitude and vertical speed errors are similar across all three BlueSky-based configurations. BADA~4 is the clear recommended model for applications requiring accurate fuel burn and emissions estimation, as it eliminates BADA~3's systematic overestimation of fuel flow ($+77\%$ error reduced to $-1\%$), reduces thrust prediction errors by $83\%$ relative to BADA~3 (pyBADA), and achieves near-perfect mass tracking ($154$\,kg vs $1{,}937$\,kg MAE). The \texttt{MASS} stack command provides a mechanism to mitigate initial mass offset by allowing the user to initialise aircraft at the exact operating mass assumed by the reference tool.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{dyn_fig05_mae_stripplots.png}
    \caption{Mean Absolute Error (MAE) distribution strip plots (with horizontal jitter) vs.\ Dynamo reference across all 26 retained flights, for altitude, VS, TAS, Mach, thrust, FF, and mass. Each point represents an individual flight; the horizontal dark bars show the median value for each model. The dramatic reduction in thrust and fuel flow error from BlueSky to BADA~4 (pyBADA) is clearly visible.}
    \label{fig:mae_stripplots}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{(Top) Total fuel burn per flight for all four models and the Dynamo reference. (Bottom) Relative fuel burn error percentage vs.\ Dynamo reference. BADA~3 overestimates by $+76.61\%$ on average; BADA~4 achieves a mean error of $-1.05\%$.}
...
\begin{figure}[H]
    ...
    \caption{Global campaign MAE summary: mean absolute error vs.\ Dynamo reference across all six primary variables plotted in individual subplots to handle differing scales, averaged over the 26 retained flights. Error bars represent one standard deviation. BADA~4 (pyBADA) achieves the lowest mean and standard deviation for thrust, fuel flow, and mass, confirming its superiority as an engine performance model.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{Flowchart_dynamic_plugin_loop.png}
    \caption{Flowchart of the per-tick update loop and state override sequence.}
    \label{fig:update_loop}
\end{figure}
...
\subsection{Validation of Point-Mass Dynamics (DYNMODE 1)}
\label{sec:validation_dynmode1}

Following the verification and analysis of the passive observer mode, this section evaluates the performance and trajectory fidelity of the plugin under active state overrides, specifically within \texttt{DYNAMIC\_BADA} Mode~1 (Point-Mass Dynamics). In this mode, the simulated flight path is no longer kinematically clamped to the waypoint constraints. Instead, the rate of climb or descent (ROCD) derived from pyBADA's energy equation is actively written back to \texttt{bs.traf.vs[i]}, and the longitudinal acceleration ($a_x$) obtained from the force balance is written to \texttt{bs.traf.ax[i]} to drive speed integration via the patched \texttt{update\_airspeed()} function. 

Evaluating the plugin in Mode~1 is essential to understand the physical and logical couplings introduced by closing the loop between the performance equations and the aircraft state vector. As established in the simulation setup, the campaign comprises the same representative European routes, with the focus set on comparing the pyBADA models against the Dynamo reference, as the baseline kinematic model deviations have already been documented in Section~\ref{sec:phase_detection}.

\subsubsection{Simulation Setup and Retained Flights}

The validation campaign under DYNMODE~1 was conducted using the same 27 gate-to-gate scenarios. However, the closed-loop dynamic integration increases the system's sensitivity to model mismatch and state machine thresholds, leading to the following outlier filtering results:
\begin{itemize}
    \item \textbf{\texttt{LEIB-LEPA} Outlier:} Discarded due to a range shortfall in the baseline BlueSky model (simulated range is less than 85\% of the Dynamo reference), identical to the MODE~0 case.
    \item \textbf{\texttt{EGHI-EGJA} Outlier:} Discarded because the BADA~4 (pyBADA) configuration produced a True Airspeed (TAS) Mean Absolute Error (MAE) exceeding 100\,kt. 
\end{itemize}
Removing these two flights leaves 25 retained scenarios for the global campaign statistics, ensuring a consistent and representative fleet comparison.

\subsubsection{Vertical Profile and Speed Divergence}

The altitude and vertical speed profiles for representative routes under DYNMODE~1 are presented in Figure~\ref{fig:altitude_profiles_dyn1}, and the corresponding airspeed profiles are shown in Figure~\ref{fig:speed_profiles_dyn1}. These profiles reveal significant vertical path divergence and speed mismatches compared to the geometry-bound MODE~0 results.

\paragraph{Steep Climb and Altitude Capture Mismatch.}
During the Climb phase, pyBADA commands maximum climb thrust (MCMB) and applies the BADA Energy Share Factor (ESF) to split excess engine power between climb rate and acceleration. In DYNMODE~1, this computed ROCD directly drives the vertical trajectory. Because there are no intermediate altitude restrictions or VNAV vertical paths in the simulator's climb autopilot, the aircraft climbs at its maximum physical performance. This results in a much steeper climb gradient and higher rate of climb compared to the Dynamo reference, which employs a VNAV-optimised cost-index profile with reduced climb thrust settings.

This steep climb introduces a critical logical coupling with the three-phase persistence-filter state machine. A transition from Climb to Cruise requires the aircraft to enter the altitude capture band ($|h - h_{\text{AP}}| \le 500$\,ft) and maintain a low vertical rate ($|VS| \le 150$\,ft/min) for a sustained window ($\tau_{\text{cruise}} = 90$\,s). However, because the aircraft climbs so rapidly under MCMB thrust (often exceeding 3,000--4,000\,ft/min), it shoots through the altitude capture band extremely fast. Before the autopilot can decelerate or stabilize the aircraft, the altitude frequently overshoots the target $h_{\text{AP}}$ by more than 500\,ft. Once the aircraft is above the capture band ($h - h_{\text{AP}} > 500$\,ft), it can no longer satisfy the cruise transition criteria and remains locked in the Climb phase. As a consequence, it continues climbing at maximum climb thrust, reaching unphysically high altitudes (up to 46,254\,ft for BADA~4) before the thrust decays and the aircraft stabilizes. This vertical path divergence explains the large altitude MAEs observed: $8{,}306.52 \pm 8{,}029.73$\,ft for BADA~3 and $6{,}857.91 \pm 7{,}339.67$\,ft for BADA~4, compared to the geometry-clamped base model ($599.02 \pm 605.12$\,ft).

\paragraph{Longitudinal Force-Balance Coupling.}
The active override of longitudinal acceleration ($a_x = \frac{T - D - W \sin \gamma}{m}$) and its integration into the airspeed state introduces additional speed errors. As the vertical profile diverges, the flight path angle ($\gamma$) also deviates. This feeds back into the longitudinal force balance via the gravitational component ($W \sin \gamma$). A steeper climb increases the gravity contribution, decelerating the aircraft, while an overshoot reduces it, coupling the speed and altitude errors. The resulting TAS MAE increases to $29.96 \pm 12.61$\,kt for BADA~3 and $24.97 \pm 23.83$\,kt for BADA~4, whereas the base BlueSky model (which does not override speed) maintains a low error of $11.85 \pm 11.44$\,kt. The Mach MAE follows a similar trend, reaching $0.049 \pm 0.023$ for BADA~3 and $0.046 \pm 0.050$ for BADA~4.

\subsubsection{Thrust and Fuel Consumption}

The thrust and fuel flow profiles are visualised in Figure~\ref{fig:thrust_fuel_profiles_dyn1}, and the mass depletion curves are shown in Figure~\ref{fig:mass_profiles_dyn1}. 

Despite the trajectory and speed divergence, BADA~4 (pyBADA) continues to demonstrate superior physical representation of engine performance compared to BADA~3 and the baseline model. The global thrust MAE is $15{,}073.69 \pm 2{,}203.71$\,daN for BlueSky base, $5{,}658.32 \pm 762.08$\,daN for BADA~3 (pyBADA), and $1{,}714.10 \pm 626.07$\,daN for BADA~4 (pyBADA) (Figure~\ref{fig:global_summary_dyn1}). BADA~4 achieves a $69.7\%$ error reduction relative to BADA~3. The fuel flow MAE shows a similar trend: $7{,}187.04 \pm 1{,}082.88$\,kg/h for BlueSky base, $2{,}768.30 \pm 531.14$\,kg/h for BADA~3, and $964.63 \pm 221.45$\,kg/h for BADA~4, representing a $65.2\%$ improvement of BADA~4 over BADA~3. This is further reflected in mass tracking, where BADA~4 achieves a mass MAE of $1{,}259.56 \pm 599.85$\,kg vs. BADA~3's $4{,}292.85 \pm 2{,}774.34$\,kg.

\subsubsection{Fuel Burn and Cumulative Error Bias}

The cumulative fuel burn per flight is plotted in Figure~\ref{fig:fuel_burn_bars_dyn1}, and the scatter and relative error scaling plots are shown in Figures~\ref{fig:fuel_scatter_dyn1} and~\ref{fig:fuel_error_vs_duration_dyn1}.

Across the 25 retained flights, the mean fuel burn errors relative to Dynamo are:
\begin{itemize}
    \item \textbf{BlueSky (base):} $+332.03\% \pm 27.41\%$
    \item \textbf{BADA~3 (pyBADA):} $+157.92\% \pm 13.14\%$
    \item \textbf{BADA~4 (pyBADA):} $+57.63\% \pm 23.04\%$
\end{itemize}

While BADA~4 remains significantly more accurate than BADA~3 and the baseline, its mean fuel burn error increased from $-1.05\%$ in MODE~0 to $+57.63\%$ in MODE~1. This positive fuel burn bias is a direct consequence of the vertical path divergence: because the aircraft climbs under maximum climb thrust (MCMB) and overshoots the target altitude, it spends a much larger fraction of the flight in the high-fuel-flow Climb phase. The lack of a refined VNAV climb schedule and the absence of a robust altitude-capture controller in the simulator's autopilot prevent the aircraft from transitioning to the low-fuel-flow Cruise phase at the correct time, accumulating a substantial positive fuel burn offset.

\subsubsection{Quantitative Validation Summary}

The per-flight normalised MAE heatmap is shown in Figure~\ref{fig:mae_heatmap_dyn1}, and the campaign-level MAE summary is consolidated in Figure~\ref{fig:global_summary_dyn1}. The main quantitative findings from the 25 retained flights are:
\begin{enumerate}
    \item \textbf{Altitude}: BlueSky (base) achieves $599.02 \pm 605.12$\,ft, BADA~3 (pyBADA) $8{,}306.52 \pm 8{,}029.73$\,ft, and BADA~4 (pyBADA) $6{,}857.91 \pm 7{,}339.67$\,ft. While the base model remains kinematically constrained to the geometric waypoint altitude targets, the pyBADA implementations diverge significantly because their ROCD is derived from BADA's physical Energy Share Factor and climb power settings.
    \item \textbf{Vertical speed}: BlueSky (base) $290.91 \pm 300.05$\,ft/min, BADA~3 (pyBADA) $2{,}443.96 \pm 411.21$\,ft/min, BADA~4 (pyBADA) $892.09 \pm 321.65$\,ft/min. The much higher errors in pyBADA models reflect the maximum-climb thrust acceleration profiles, with BADA~4 demonstrating a more stable climb profile compared to BADA~3.
    \item \textbf{True airspeed}: BlueSky (base) $11.85 \pm 11.44$\,kt, BADA~3 (pyBADA) $29.96 \pm 12.61$\,kt, BADA~4 (pyBADA) $24.97 \pm 23.83$\,kt. This speed mismatch in the pyBADA integrations is a direct consequence of the force-driven acceleration override, which propagates trajectory and flight phase discrepancies into the velocity state.
    \item \textbf{Thrust}: BlueSky (base) $15{,}073.69 \pm 2{,}203.71$\,daN, BADA~3 (pyBADA) $5{,}658.32 \pm 762.08$\,daN, BADA~4 (pyBADA) $1{,}714.10 \pm 626.07$\,daN. BADA~4 remains significantly more representative of the Dynamo reference, yielding a $69.7\%$ reduction in error relative to BADA~3.
    \item \textbf{Fuel flow}: BlueSky (base) $7{,}187.04 \pm 1{,}082.88$\,kg/h, BADA~3 (pyBADA) $2{,}768.30 \pm 531.14$\,kg/h, BADA~4 (pyBADA) $964.63 \pm 221.45$\,kg/h. BADA~4 achieves a $65.2\%$ reduction in fuel flow error vs BADA~3, reflecting the superior physics of the TSFC-based fuel flow formulation even under path-deviated conditions.
    \item \textbf{Mass}: BlueSky (base) $218{,}608.84 \pm 7{,}783.93$\,kg, BADA~3 (pyBADA) $4{,}292.85 \pm 2{,}774.34$\,kg, BADA~4 (pyBADA) $1{,}259.56 \pm 599.85$\,kg. BADA~4 maintains a more precise mass depletion profile, reducing mass tracking error by $70.7\%$ vs BADA~3.
\end{enumerate}

In conclusion, while the active point-mass dynamic override in DYNMODE~1 increases physical consistency by coupling thrust and aerodynamic drag to the trajectory integration, it highlights a critical limitation of the simulator's guidance system. Without a VNAV flight path calculator and an altitude-capture control law that adjusts climb thrust ratings dynamically as the aircraft approaches cruise flight levels, the closed-loop system is highly susceptible to trajectory and speed deviations. Despite these guidance limitations, BADA~4 (pyBADA) remains consistently superior to BADA~3 (pyBADA) and the base model, achieving a $69.7\%$ reduction in thrust error and a $65.2\%$ reduction in fuel flow error under dynamic conditions.

% ════════════════════════════════════════════════════════════════════════════
% DYNMODE 1 LaTeX figures definition
% ════════════════════════════════════════════════════════════════════════════

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{dyn1_fig01_altitude_profiles.png}
    \caption{Altitude [kft] and Vertical Speed (VS) profiles vs.\ flight progress for representative routes under DYNMODE~1 (ISA conditions). The active ROCD override causes the aircraft to climb under maximum climb thrust, resulting in a much steeper vertical path than the Dynamo cost-index reference and significant altitude overshoots.}
    \label{fig:altitude_profiles_dyn1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{dyn1_fig02_speed_profiles.png}
    \caption{TAS and Mach profiles vs.\ flight progress for representative routes under DYNMODE~1. The active longitudinal acceleration override couples the speed integration to the flight path angle deviations, leading to increased speed mismatches relative to the reference.}
    \label{fig:speed_profiles_dyn1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{dyn1_fig03_thrust_fuel_profiles.png}
    \caption{Thrust and Fuel Flow (FF) profiles vs.\ flight progress for representative routes under DYNMODE~1. BADA~4's TSFC-based fuel flow model remains physically consistent with the reference, but shows extended climb thrust peaks due to the prolonged climb phase.}
    \label{fig:thrust_fuel_profiles_dyn1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{dyn1_fig04_mass_profiles.png}
    \caption{Aircraft mass depletion profiles vs.\ flight progress under DYNMODE~1. The excessive fuel flow and prolonged climb in BADA~3 lead to rapid mass depletion, while BADA~4 tracks the Dynamo reference trend more closely.}
    \label{fig:mass_profiles_dyn1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{dyn1_fig05_mae_stripplots.png}
    \caption{Mean Absolute Error (MAE) distribution strip plots (with horizontal jitter) vs.\ Dynamo reference across all 25 retained flights under DYNMODE~1. The vertical and speed errors are visibly higher than the kinematic MODE~0 counterparts, but BADA~4 retains its engine performance model superiority.}
    \label{fig:mae_stripplots_dyn1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{dyn1_fig06_mae_heatmap.png}
    \caption{Per-flight normalised MAE heatmap vs.\ Dynamo reference under DYNMODE~1. The BADA~4 (pyBADA) configuration exhibits consistently lower error entries for thrust, fuel flow, and mass across the fleet.}
    \label{fig:mae_heatmap_dyn1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{dyn1_fig07_fuel_burn_bars.png}
    \caption{(Top) Total fuel burn per flight for all four models and the Dynamo reference under DYNMODE~1. (Bottom) Relative fuel burn error percentage vs.\ Dynamo reference. The vertical path divergence shifts the BADA~4 mean error to $+57.63\%$, while BADA~3 overpredicts by $+157.92\%$.}
    \label{fig:fuel_burn_bars_dyn1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{dyn1_fig08_global_summary.png}
    \caption{Global campaign MAE summary vs.\ Dynamo reference across the 25 retained flights under DYNMODE~1. BADA~4 (pyBADA) achieves the lowest errors for engine performance variables (thrust, fuel flow, mass) under active feedback.}
    \label{fig:global_summary_dyn1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{dyn1_fig09_fuel_scatter.png}
    \caption{Scatter plot of simulated fuel burn vs.\ Dynamo reference under DYNMODE~1. All models show a positive bias relative to the Dynamo reference due to the path divergence during climb.}
    \label{fig:fuel_scatter_dyn1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{dyn1_fig10_fuel_error_vs_duration.png}
    \caption{Fuel burn relative error vs.\ flight duration for all models under DYNMODE~1. The positive bias remains persistent across all route lengths, demonstrating the systematic impact of the guidance system limitations.}
    \label{fig:fuel_error_vs_duration_dyn1}
\end{figure}