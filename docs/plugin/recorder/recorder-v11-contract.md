# `samples-v11` recorder contract

## Scope

`samples-v11` is the sole supported research-recorder schema. Every CSV row and
...
| Artifact        | Path                             | Role                                                          |
|-----------------|----------------------------------|---------------------------------------------------------------|
| Samples         | `run.csv`                        | Authoritative, ordered `samples-v11` rows.                    |
...
|  # | Column                           | Unit   | Meaning                                                                                                                |
|---:|----------------------------------|--------|------------------------------------------------------------------------------------------------------------------------|
|  1 | `schema_version`                 | -      | Literal `samples-v11`.                                                                                                 |
...
| 57 | `model_rocd_m_s`                 | m/s    | Raw model ROCD before geometric-altitude propagation correction and final energy allocation.                           |
| 58 | `energy_share_factor`            | 1      | Model energy-share factor.                                                                                             |
| 59 | `energy_allocation_policy`       | -      | Applied energy-allocation policy.                                                                                      |
| 60 | `propulsion_bank_angle_deg`      | deg    | Effective bank angle supplied to propulsion/performance evaluation.                                                    |
| 61 | `propulsion_load_factor`         | 1      | Load factor supplied to propulsion/performance evaluation.                                                             |
| 62 | `envelope_policy`                | -      | Effective per-aircraft `OFF`, `REPORT`, `ENFORCE`, or `ABORT` policy.                                                  |
| 63 | `envelope_profile`               | -      | Effective envelope profile name.                                                                                       |
| 64 | `envelope_checks`                | -      | Comma-separated effective check names.                                                                                 |
| 65 | `envelope_status`                | -      | Aggregated current envelope status.                                                                                    |
| 66 | `envelope_failed_checks`         | -      | Comma-separated contributing failed checks.                                                                            |
| 67 | `envelope_last_action`           | -      | Most recent envelope action.                                                                                           |
| 68 | `envelope_last_reason`           | -      | Reason associated with the most recent emitted quality event.                                                          |
| 69 | `envelope_event_count`           | -      | Cumulative quality-event count for the aircraft.                                                                       |
| 70 | `envelope_violation_count`       | -      | Cumulative non-empty envelope-reason count for the aircraft.                                                           |
| 71 | `mass_min_kg`                    | kg     | Evaluated minimum mass bound.                                                                                          |
| 72 | `mass_max_kg`                    | kg     | Evaluated maximum mass bound.                                                                                          |
| 73 | `envelope_configuration`         | -      | Configuration used for flight-envelope bounds.                                                                         |
| 74 | `minimum_cas_m_s`                | m/s    | Evaluated minimum CAS bound.                                                                                           |
| 75 | `maximum_cas_m_s`                | m/s    | Evaluated maximum CAS bound.                                                                                           |
| 76 | `minimum_mach`                   | 1      | Evaluated minimum Mach bound.                                                                                          |
| 77 | `maximum_mach`                   | 1      | Evaluated maximum Mach bound.                                                                                          |
| 78 | `maximum_altitude_m`             | m      | Model-returned maximum-altitude bound. Its non-ISA coordinate interpretation remains a documented modeling limitation. |
| 79 | `minimum_rocd_m_s`               | m/s    | Evaluated signed minimum vertical-rate bound.                                                                          |
| 80 | `maximum_rocd_m_s`               | m/s    | Evaluated signed maximum vertical-rate bound.                                                                          |
| 81 | `envelope_lateral_configuration` | -      | Configuration used for lateral bounds.                                                                                 |
| 82 | `bank_angle_deg`                 | deg    | Effective commanded turn-bank angle.                                                                                   |
| 83 | `load_factor`                    | 1      | Coordinated level-turn load factor derived from the effective bank angle.                                              |
| 84 | `minimum_load_factor`            | 1      | Evaluated minimum load-factor bound.                                                                                   |
| 85 | `maximum_load_factor`            | 1      | Evaluated maximum load-factor bound.                                                                                   |
| 86 | `maximum_bank_angle_deg`         | deg    | Evaluated maximum bank-angle bound.                                                                                    |

Columns 51 through 57 are the v11 numerical-audit inputs. They capture the
state actually used for the TEM evaluation rather than reconstructing that
...
| Key                  | Meaning                                                                                                               |
|----------------------|-----------------------------------------------------------------------------------------------------------------------|
| `schema_version`     | Literal `samples-v11`.                                                                                                |
...
| `columns`            | Exact ordered list of the 86 CSV columns above.                                                                       |
