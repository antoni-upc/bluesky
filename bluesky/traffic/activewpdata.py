...
class ActiveWaypoint(Entity, replaceable=True):
    def __init__(self):
        ...
        with self.settrafarrays():
            ...
            self.nextspdalt  = np.array([])    # [m] Altitude of the waypoint where nextspd applies (<0: none)
            self.prevspd     = np.array([])    # [CAS[m/s]/Mach] Speed of the leg before the active one (<0: none)
    ...
    def create(self, n=1):
        ...
        self.nextspdalt[-n:] = -999.    # [m] Altitude of the waypoint where nextspd applies
        self.prevspd[-n:]    = -999.    # [CAS[m/s]/Mach] Speed of the leg before the active one
