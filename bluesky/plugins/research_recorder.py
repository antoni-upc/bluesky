...
def atmosstatus(acid=None):
    ...
    for idx in indices:
        ...
        lines.append(
            f'{bs.traf.id[idx]}: source={traffic_value("atmos_source", "ISA")} '
            f'valid={bool(traffic_value("atmos_valid", True))} '
            f'dataset={traffic_value("atmos_dataset_time", "") or "-"} '
            f'fallback={traffic_value("atmos_fallback_reason", "") or "-"}\n'
            f'  geometric_alt={bs.traf.alt[idx]:.1f} m '
            f'pressure_alt={traffic_value("pressure_alt", bs.traf.alt[idx]):.1f} m\n'
