...
FIELDS = (
    ...
    'required_thrust_n', 'rated_thrust_n', 'idle_thrust_n', 'maximum_thrust_n',
    'drag_n', 'fuel_flow_kg_s', 'mass_kg', 'target_tas_m_s',
...
UNITS = {
    ...
    'wind_east_m_s': 'm/s', 'thrust_n': 'N', 'required_thrust_n': 'N',
    'rated_thrust_n': 'N', 'idle_thrust_n': 'N', 'maximum_thrust_n': 'N', 'drag_n': 'N',
...
class StreamingRecorder:
    ...
    def sample(self):
        ...
        for idx, acid in enumerate(bs.traf.id):
            def traffic_value(name, default):
                values = getattr(bs.traf, name, None)
                return default if values is None or idx >= len(values) else values[idx]

            source = traffic_value('atmos_source', 'ISA')
            dataset_time = traffic_value('atmos_dataset_time', '')
            if source:
            ...
            row = {
                ...
                'pressure_alt_m': traffic_value('pressure_alt', bs.traf.alt[idx]),
                'tas_m_s': bs.traf.tas[idx],
                ...
                'atmosphere_valid': bool(traffic_value('atmos_valid', True)),
                'dataset_time': dataset_time,
                'fallback_reason': traffic_value('atmos_fallback_reason', ''),
                ...
                'required_thrust_n': perf_value('required_thrust'),
                'rated_thrust_n': perf_value('rated_thrust'),
                'idle_thrust_n': perf_value('idle_thrust'),
                'maximum_thrust_n': perf_value('maximum_thrust'),
                'drag_n': perf_value('drag'), 'fuel_flow_kg_s': perf_value('fuelflow'),
                'mass_kg': perf_value('mass'),
                'target_tas_m_s': perf_value('target_tas'),
                'requested_acceleration_m_s2': perf_value('requested_acceleration'),
                'applied_acceleration_m_s2': perf_value('applied_acceleration'),
                'thrust_limited': perf_value('thrust_limited'),
                'thrust_limitation_reason': perf_value('thrust_limitation_reason'),
