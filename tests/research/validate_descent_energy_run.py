#!/usr/bin/env python3
"""Validate licensed simultaneous deceleration/descent BADA energy evidence."""
...
def validate(path, family, power_tolerance=0.75, motion_tolerance=0.08):
    ...
    if metadata.get('scenario') != f'pybada-descent-energy-bada{family}':
    ...
    if not required.issubset(set(metadata.get('columns', ()))):
        errors.append('metadata lacks joint-energy fields')
    ...
    stable = [row for row in ordered
              if _number(row, 'sim_time_s') > 125.0
              and abs(_number(row, 'vertical_speed_m_s')) <= 1e-6
              and _true(row, 'speed_capture')]
    if len(joint) < 100:
        errors.append(f'insufficient simultaneous deceleration/descent evidence: {len(joint)}')
    if len(stable) < 20:
        errors.append(f'insufficient stable capture evidence: {len(stable)}')

    residuals, ax_errors, vs_errors, mass_errors = [], [], [], []
    joint_ids = {id(row) for row in joint}
    ...
    if max(residuals, default=math.inf) > power_tolerance:
        errors.append(f'maximum signed total-energy residual '
    ...
    if any(not (_number(row, 'idle_thrust_n') - 1e-6 <=
                ...
                _number(row, 'maximum_thrust_n') + 1e-6) for row in joint):
        errors.append('joint samples exceed recorded thrust bounds')
    ...
    commanded_altitude = 10000.0 * 0.3048
    if final is not None:
        if abs(_number(final, 'geometric_alt_m') - commanded_altitude) > 0.02:
            errors.append(f'aircraft did not capture 10000 ft: '
    ...
    return (f'VALID: {len(rows)} BADA {family} descent-energy rows; '
            f'joint={len(joint)}, stable={len(stable)}, '
