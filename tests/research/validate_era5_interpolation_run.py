#!/usr/bin/env python3
"""Validate opt-in ERA5 temporal interpolation during matched TEM flight."""
...
PROVENANCE = re.compile(
    r'^2025-08-15T12:00:00->2025-08-15T13:00:00@(?P<weight>0\.\d{6})$')


def validate(path, cache='cache/weather/era5'):
    path = Path(path)
    ...
    if set(by_acid) != {'EITR', 'EITO'}:
        errors.append(f'aircraft are {sorted(by_acid)}, expected EITR and EITO')
    ...
    slot1 = datetime(2025, 8, 15, 13, tzinfo=timezone.utc)
    first = provider._read(provider._path(slot0, bounds, 0), slot0.replace(tzinfo=None))
    second = provider._read(provider._path(slot1, bounds, 0), slot1.replace(tzinfo=None))

    for acid, samples in by_acid.items():
        expected_policy = 'REPORT' if acid == 'EITR' else 'OFF'
        ...
        for row in samples:
            ...
            expected_weight = (utc - utc.replace(minute=0, second=0, microsecond=0)).total_seconds() / 3600.0
    ...
    left = {row['sim_time_s']: row for row in by_acid['EITR']}
    right = {row['sim_time_s']: row for row in by_acid['EITO']}
    ...
    if errors:
        return 'INVALID ERA5 interpolation evidence:\n  - ' + '\n  - '.join(errors)
    return (f'VALID: {len(rows)} samples, UTC-derived ERA5 temporal interpolation; '
...
def main():
    try:
        ...
        result = f'INVALID ERA5 interpolation evidence:\n  - evidence could not be validated: {exc}'
    print(result)
