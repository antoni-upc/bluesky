...
def check_transition(acid, row, events, policy, status, action, config, errors):
    check_resolution(acid, row, errors)
    if row.get('envelope_lateral_configuration') != config:
        errors.append(f'{acid} lateral configuration is not {config}')
...
def validate(path, terminal=False, abort=False, terminal_abort=False):
    ...
    if abort or terminal_abort:
        ...
        if row is None:
            ...
            if row.get('envelope_lateral_configuration') != expected_config:
                errors.append(f'{acid} lateral configuration is not {expected_config}')
        matches = [event for event in events if event.get('aircraft') == acid
                   and event.get('reason') == 'BANK_ANGLE,LOAD_FACTOR'
        ...
        for acid, values in expected.items():
            ...
            if row is None:
                ...
                continue
            check_transition(acid, row, events, *values, errors)
