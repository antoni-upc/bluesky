...
def test_recorder_package_imports_without_meteo_or_pybada():
    script = (
        ...
        "assert recorder.SCHEMA_VERSION == 'samples-v12'; "
...
def test_recorder_streams_and_resets_without_retaining_rows(tmp_path, monkeypatch):
    ...
    traffic.perf.evaluation_speed_evolution = np.array(['constM'])
    traffic.perf.evaluation_speed_target_cas = np.array([np.nan])
    traffic.perf.evaluation_speed_target_mach = np.array([0.78])
    traffic.perf.evaluation_speed_target_tas = np.array([np.nan])
    recorder.sample()
    ...
    assert row['evaluation_speed_evolution'] == ''
    assert row['evaluation_speed_target_cas_m_s'] == ''
    assert row['evaluation_speed_target_mach'] == ''
    assert row['evaluation_speed_target_tas_m_s'] == ''
    assert second_row['evaluation_speed_evolution'] == 'constM'
    assert second_row['evaluation_speed_target_mach'] == '0.78'
    position = FIELDS.index('evaluation_timestep_s') + 1
    assert FIELDS[position:position + 4] == (
...
def test_recorder_uses_isa_defaults_without_meteorology_hooks(tmp_path, monkeypatch):
    values = np.array([1.0])
    traffic = SimpleNamespace(
        id=['BASE'], type=['A320'], lat=values, lon=values, alt=values,
        tas=values, cas=values, M=values, vs=values, hdg=values, trk=values,
        Temp=np.array([288.0]), p=np.array([101325.0]), rho=np.array([1.225]),
        windnorth=np.array([0.0]), windeast=np.array([0.0]),
        perf=SimpleNamespace(thrust=values, drag=values, fuelflow=values, mass=values))
    monkeypatch.setattr(bs, 'traf', traffic)
    monkeypatch.setattr(bs, 'sim', SimpleNamespace(
        simt=0.0, simdt=0.05, utc=datetime(2026, 1, 1, tzinfo=timezone.utc)))
    monkeypatch.setattr('bluesky.plugins.recorder.streaming.stack.get_scenname',
                        lambda: 'standalone')
    recorder = StreamingRecorder()
    recorder.start(tmp_path / 'standalone.csv')
    recorder.sample()
    csv_path, _ = recorder.stop()

    with csv_path.open(newline='') as stream:
        row = next(csv.DictReader(stream))
    assert row['atmosphere_source'] == 'ISA'
    assert row['atmosphere_valid'] == 'True'
    assert row['dataset_time'] == ''
    assert row['fallback_reason'] == ''
    assert row['pressure_alt_m'] == row['geometric_alt_m']
