...
from bluesky.plugins.pybada.envelope import (
    EnvelopeCheck, EnvelopePolicy, EnvelopeProfile, EnvelopeStatus,
    evaluate_mass, expand_checks, mass_bounds, parse_checks)
...
def make_perf(policies=('OFF', 'OFF')):
    ...
    perf.envelope_profile = np.asarray(['LONGITUDINAL'] * len(policies), dtype='U12')
    perf.envelope_checks = [expand_checks(EnvelopeProfile.LONGITUDINAL) for _ in policies]
...
def traffic(monkeypatch):
    monkeypatch.setattr(bs, 'traf', SimpleNamespace(id=['A1', 'A2']))
